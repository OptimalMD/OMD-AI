...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-6">
	<div
		...
		class="mx-4 lg:ml-4 lg:mr-4 mt-4 mb-4 flex flex-row overflow-x-auto gap-2 max-w-full lg:flex-col lg:flex-none lg:w-52 text-sm font-medium scrollbar-none bg-gray-50 dark:bg-gray-900 p-2 rounded-lg"
	>
		<button
			id="general"
			class="px-4 py-2.5 min-w-fit rounded-lg lg:w-full flex items-center gap-3 text-left transition {selectedTab ===
			'general'
		...
		>
			<DocumentChartBar className="size-4 flex-shrink-0" />
			<span>{$i18n.t('Evaluations')}</span>
		</button>
		...
		>
			...
			<span>{$i18n.t('External Tools')}</span>
		</button>
		...
		>
			...
			<span>{$i18n.t('Code Execution')}</span>
		</button>
